...
// node_modules/hunspell-spellchecker/lib/dictionary.js
var require_dictionary = __commonJS({
  "node_modules/hunspell-spellchecker/lib/dictionary.js"(exports2, module2) {
    ...
    Dictionary.prototype._parseDIC = function(data) {
      ...
      function addWord(word2, rules) {
        if (!(word2 in dictionaryTable) || typeof dictionaryTable[word2] != "object") {
...
// node_modules/hunspell-spellchecker/lib/index.js
var require_lib = __commonJS({
  "node_modules/hunspell-spellchecker/lib/index.js"(exports2, module2) {
    var Dictionary = require_dictionary();
    var Spellchecker2 = function(dictionary) {
      this.dict = null;
      if (dictionary) this.use(dictionary);
    };
    Spellchecker2.prototype.use = function(dictionary) {
      this.dict = new Dictionary(dictionary);
    };
    Spellchecker2.prototype.parse = function(dictionary) {
      var dict = new Dictionary();
      dict.parse(dictionary);
      this.use(dict);
      return dict.toJSON();
    };
    Spellchecker2.prototype.check = function(aWord) {
    ...
    Spellchecker2.prototype.checkExact = function(word) {
    ...
    Spellchecker2.prototype.hasFlag = function(word, flag, wordFlags) {
    ...
    Spellchecker2.prototype.suggest = function(word, limit) {
      if (!limit) limit = 5;
      ...
      var self2 = this;
      self2.dict.alphabet = "abcdefghijklmnopqrstuvwxyz";
      function edits1(words) {
        var rv = [];
        for (var ii = 0, _iilen = words.length; ii < _iilen; ii++) {
          var word2 = words[ii];
          var splits = [];
          for (var i2 = 0, _len2 = word2.length + 1; i2 < _len2; i2++) {
            splits.push([word2.substring(0, i2), word2.substring(i2, word2.length)]);
          }
          var deletes = [];
          for (var i2 = 0, _len2 = splits.length; i2 < _len2; i2++) {
            var s = splits[i2];
            if (s[1]) {
          ...
          for (var i2 = 0, _len2 = splits.length; i2 < _len2; i2++) {
            var s = splits[i2];
            if (s[1]) {
              for (var j = 0, _jlen = self2.dict.alphabet.length; j < _jlen; j++) {
                replaces.push(s[0] + self2.dict.alphabet[j] + s[1].substring(1));
              }
            }
          }
          var inserts = [];
          for (var i2 = 0, _len2 = splits.length; i2 < _len2; i2++) {
            ...
            if (s[1]) {
              for (var j = 0, _jlen = self2.dict.alphabet.length; j < _jlen; j++) {
                replaces.push(s[0] + self2.dict.alphabet[j] + s[1]);
              }
            }
          }
      ...
      function correct(word2) {
        ...
        for (var i2 = 0, _len2 = corrections.length; i2 < _len2; i2++) {
          if (!(corrections[i2] in weighted_corrections)) {
            weighted_corrections[corrections[i2]] = 1;
          } else {
            weighted_corrections[corrections[i2]] += 1;
          }
        }
        var sorted_corrections = [];
        for (var i2 in weighted_corrections) {
        ...
        var rv = [];
        for (var i2 = 0, _len2 = Math.min(limit, sorted_corrections.length); i2 < _len2; i2++) {
          if (!self2.hasFlag(sorted_corrections[i2][0], "NOSUGGEST")) {
...
var require_axios = __commonJS({
  "node_modules/axios/dist/node/axios.cjs"(exports2, module2) {
    ...
    function renderKey2(path10, key, dots) {
    ...
    function toFormData3(obj, formData, options) {
      ...
      function defaultVisitor(value, key, path10) {
        ...
        formData.append(renderKey2(path10, key, dots), convertValue(value));
        return false;
      }
      const stack = [];
    ...
    axios3.all = function all3(promises2) {
    ...
    axios3.formToJSON = (thing) => formDataToJSON2(utils$1.isHTMLForm(thing) ? new FormData(thing) : thing);
...
function createWebviewPanel(title, description, fileListOccurrences, context, onCommand2, showLineNumbers = true, headerTitle = "Possible Detected Occurrences") {
...
function getWebviewContent(title, description, fileListOccurrences, context, showLineNumbers, headerTitle) {
  ...
  const totalCount = aiDetectedOccurrences.reduce(
    ...
    (file, fileIndex) => `
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
          ...
          <div class="p-4">
            <table class="w-full">
              <tbody>
                ${file.occurrences.map(
      (occ, occIndex) => `
                  <tr class="${(occ.options || {}).disable ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-100 cursor-pointer"}"
                    data-ai="${occ.ai ? "true" : "false"}"
                      data-file-index="${fileIndex}"
                      data-occ-index="${occIndex}">
                    <td class="py-2 px-4 border-b flex items-center">
                      ${occ.ai ? '<svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3 3 0 013.75-2.906z"></path></svg>' : ""}
                      ${escapeHtml(occ.text)}
                    </td>
                    ${showLineNumbers ? `<td class="py-2 px-4 border-b text-right">${occ.line}</td>` : ""}
                  </tr>
                `
    ).join("")}
                </tbody>
        ...
        `
          ...
          ${possibleDetectedOccurrences.length > 0 ? `<h2 class="text-xl font-bold mt-4 mb-2">${headerTitle}</h2>` : ""}
...
function jumpToFileLine(message) {
  ...
  vscode2.workspace.openTextDocument(uri).then((doc) => {
    vscode2.window.showTextDocument(doc, { viewColumn: vscode2.ViewColumn.One }).then((editor) => {
      const position = new vscode2.Position(message.line - 1, 0);
...
function jumpToLocaleKey(message) {
  try {
    const filePath = message.filePath;
    const config = vscode2.workspace.getConfiguration("i18nAiExtractor");
    const localeFolder = config.get("localeResourceFolder", "locale");
    const projectPath = vscode2.workspace.workspaceFolders[0].uri.fsPath;
    const relativePath = import_path.default.relative(
      import_path.default.join(projectPath, localeFolder),
      filePath
    );
    const uniqueIdentifier = relativePath.replace(/\.\w+$/, "").replace(/\//g, ".");
    const parts = message.text.split(" - ");
    const key = parts[parts.length - 1];
    const localeKey = key.substring(uniqueIdentifier.length + 1);
    const localeFileContent = (0, import_fs.readFileSync)(filePath, "utf8");
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (match) {
        const key2 = match[1];
        if (key2 === keyParts[keyMatchedIndex]) {
          keyMatchedIndex++;
    ...
    vscode2.workspace.openTextDocument(uri).then((doc) => {
      vscode2.window.showTextDocument(doc, { viewColumn: vscode2.ViewColumn.One }).then((editor) => {
        const position = new vscode2.Position(lineNumber, 0);
        editor.selection = new vscode2.Selection(position, position);
        editor.revealRange(new vscode2.Range(position, position));
      });
    });
...
// utils/unlocalized_scanner.js
var vscode3 = __toESM(require("vscode"));
function scanForUnlocalizedText(code, fileType) {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    let match;
    stringRegex.lastIndex = lineMatchStartIndex;
...
function shouldBeLocalized(text, currentLine) {
  ...
  if (/^(className|style|width|height|id|src|alt|href|type|placeholder|value)$/.test(
    text
  )) {
    return false;
  }
  if (/^(bg-|text-|border-|p-|m-|flex|grid|col-|row-|sm:|md:|lg:|xl:|2xl:)/.test(
    text
  )) {
    return false;
  }
  ...
  return text.length > 1 && text !== '""' && text !== "''" && /[A-Za-z]/.test(text) && // Contains at least one letter
...
function isEqualExpression(code, text, originalIndex) {
  ...
  return equalityPattern.test(beforeText) || afterEqualityPattern.test(afterText);
}
function isInsideI18next(currentLine, previousLine) {
  const config = vscode3.workspace.getConfiguration("i18nAiExtractor");
  const i18nDetectPrefixNames = config.get("i18nDetectPrefixNames", "i18n.t,t").split(",").map((name) => name.trim());
  const i18nRegex = new RegExp(
    `(${i18nDetectPrefixNames.join("|")})\\s*\\(\\s*['"]`
  );
  return previousLine && i18nDetectPrefixNames.some(
    (name) => previousLine.trim().endsWith(`${name}(`)
...
function isInsideComplexProp(code, text, index) {
  ...
  const textBetweenBraces = beforeText.slice(lastOpenBrace + 1) + text + afterText.slice(0, nextCloseBrace);
...
function isIgnoredText(text) {
  return isCamelCase(text) || isSnakeCase(text) || isDotExpression(text) || isNumber(text) || isPath(text) || isColor(text) || isHTMLCode(text);
}
...
function isHTMLCode(text) {
  ...
  if (headMatch && tailMatch && headMatch[1] === tailMatch[1] || text.endsWith("/>")) {
    return true;
  }
}
  ...
  if (["jsx", "js", "html"].includes(fileType)) {
    ...
    while ((match = jsxTagRegex.exec(code)) !== null) {
      ...
      if (beforeCloseText.indexOf(">") > -1) {
        ...
        matchIndex = matchIndex + allMatch.indexOf(">") + beforeCloseText.lastIndexOf(">") + 2 + ((/^(\s|\r|\n)+/.exec(text) || [""])[0] || {}).length || 0;
        text = text.trim();
      }
      if (openingTag === closingTag && text.indexOf("<") < 0 && text.indexOf("</") < 0 && text.indexOf("/>") < 0 && text.indexOf("<") < 0 && text.indexOf("}") < 0 && text.indexOf("{") < 0 && text.indexOf("(") < 0 && text.indexOf(")") < 0 && text.indexOf("=") < 0 && shouldBeLocalized(text, fileType)) {
        const lineNumber = code.slice(0, matchIndex).split("\n").length;
        const currentLineText = code.split("\n")[lineNumber - 1];
        if (!isIgnoredText(text) && !unlocalizedTexts.find((t) => t.text === text)) {
          unlocalizedTexts.push({
            text,
            index: matchIndex,
            line: lineNumber,
    ...
    while ((multiLineStringMatch = multiLineStringRegex.exec(code)) !== null) {
      ...
      if (shouldBeLocalized(text, fileType)) {
        const lineNumber = code.slice(0, multiLineStringMatch.index).split("\n").length;
        const currentLineText = code.split("\n")[lineNumber - 1];
        if (!isIgnoredText(text) && !unlocalizedTexts.find((t) => t.text === text)) {
          unlocalizedTexts.push({
            text,
...
var cookies_default = platform_default.hasStandardBrowserEnv ? (
  // Standard browser envs support document.cookie
  {
    write(name, value, expires, path10, domain, secure) {
      ...
      utils_default.isString(path10) && cookie.push("path=" + path10);
...
function createOutputChannel() {
  outputChannel = outputChannel || vscode4.window.createOutputChannel("i18n AI Extractor");
...
async function findUnlocalizedText(text, config) {
  ...
  return await vscode5.window.withProgress(
    {
    ...
    async (progress) => {
      ...
      try {
        ...
        outputChannel2.appendLine("AI Response:");
        outputChannel2.appendLine(aiOutput);
        outputChannel2.show();
...
function parseAIOutput(aiOutput, fileContent, config) {
  ...
  try {
    ...
    const fixedOutput = cleanedOutput.replace(/([{,]\s*)(\w+)(\s*:)/g, '$1"$2"$3').replace(/'/g, '"').replace(/,\s*}/g, "}").replace(/\n/g, "");
    try {
  ...
  unlocalizedTexts = parsedOutput.map((item) => {
    ...
    return result;
  }).filter((item) => item.line !== -2).filter(
    (item, index, self2) => index === self2.findIndex(
      (t) => t.text === item.text
      // Compare text property for duplicates
    )
  );
  console.log(
...
function findTextLineNumber(text, fileLines, i18nFunctionName) {
  ...
  if (matchLine > -1) {
    const surroundingLines = fileLines.slice(Math.max(0, matchLine - 2), matchLine + 3).join("\n");
    const i18nRegex = new RegExp(
...
function trimCode(code, maxSize) {
  ...
  for (const line of lines) {
    ...
    const truncatedLine = trimmedLine.length > 100 ? trimmedLine.substring(0, 97) + "..." : trimmedLine;
    if (currentSize + truncatedLine.length + 1 > maxSize) {
      trimmedLines.push("// ... (truncated)");
      break;
    }
    trimmedLines.push(trimmedLine);
    currentSize += truncatedLine.length + 1;
  }
...
async function scanSingleFile(filePath, config, executor) {
  ...
  const fileExt = import_path2.default.basename(filePath).split(".").slice(1).join(".");
  ...
  const code = (0, import_fs2.readFileSync)(filePath, "utf8");
  const fileRelativePath = import_path2.default.relative(
  ...
  patterns.forEach((pattern) => {
    ...
    while ((match = pattern.exec(code)) !== null) {
      ...
      if (key.includes(";")) {
        ...
        const wildcardKey = keyParts.map(
          (part) => part.includes("${") || part.includes("}") || part === "" ? "*" : part
        ).join(".").replace(/(\.\*)+$/, ".*");
        keys.add(wildcardKey);
      }
    }
  });
  const customAttrPattern = /[''](label-i18n|title-i18n)["']\s*:\s*['']([^"']+)['']/g;
  let customAttrMatch;
...
var collectAllI18nKeys = {
  bind(context, config) {
    return ({ code, filePath, fileType, fileRelativePath }) => {
      const localeFolder = config.get("localeResourceFolder", "locale");
      const relativePath = import_path3.default.relative(localeFolder, fileRelativePath);
      const uniqueIdentifier = relativePath.replace(/\.\w+$/, "").replace(/\//g, ".");
      const keysWithPaths = /* @__PURE__ */ new Map();
      let locales;
      try {
      ...
      return Array.from(keysWithPaths, ([key, filePath2]) => ({
        key,
        filePath: filePath2
      }));
    };
      ...
      try {
        ...
        await vscode9.window.withProgress(
          ...
          async (progress, token) => {
            ...
            const scanFoldersForUnused = config.get("scanFolderForUnused", "src").split(",").map((folder) => folder.trim());
            const folderPaths = scanFoldersForUnused.map(
              (folder) => path5.join(vscode9.workspace.workspaceFolders[0].uri.fsPath, folder)
            );
            let usedI18nKeys = [];
            ...
            usedI18nKeys = Array.from(new Set(usedI18nKeys));
            progress.report({
              message: "Scanning for defined i18n keys"
            });
            const localeFolder = config.get("localeResourceFolder", "locale");
            const localeFolderPath = path5.join(
            ...
            let unusedI18nKeys = allI18Keys.filter(
              (item) => !usedI18nKeys.some(
                (usedKey) => usedKey && (usedKey === item.key || usedKey.endsWith(".*") && item.key.startsWith(usedKey.replace(".*", "")))
              )
            );
            ...
            let totalFiles = [];
            for (const folderPath of folderPaths) {
              const t = await vscode9.workspace.findFiles(
                new vscode9.RelativePattern(folderPath, filePattern)
              );
            ...
            for (const file of totalFiles) {
              ...
              const fileContent = await vscode9.workspace.fs.readFile(file);
        ...
        if (confirmedUnusedI18nKeys.length > 0) {
          const resultReducer2 = (line) => (acc, item) => {
          ...
          const results = confirmedUnusedI18nKeys.reduce(resultReducer2(1), []);
          ...
          vscode9.window.showInformationMessage("No unused i18n keys found.");
        }
      } catch (error) {
...
// utils/typo_scanner.js
var vscode10 = __toESM(require("vscode"));
var fs3 = __toESM(require("fs"));
var path6 = __toESM(require("path"));
var import_hunspell_spellchecker = __toESM(require_lib());
var ignoreWords = null;
var flagWords = null;
function resetCSPellSetting() {
  ignoreWords = null;
  flagWords = null;
}
async function scanTyposInLocalFiles({ code, fileType }) {
  if (ignoreWords === null) {
    const workspaceFolders = vscode10.workspace.workspaceFolders;
    if (workspaceFolders) {
      const cspellPath = path6.join(
        workspaceFolders[0].uri.fsPath,
        "cspell.json"
      );
      try {
        const cspellContent = await fs3.promises.readFile(cspellPath, "utf8");
        const cspellTextContent = String(cspellContent).split("\n").filter((m) => !m.trim().startsWith("//")).join("\n");
  ...
  const spellchecker = new import_hunspell_spellchecker.default();
  const dictionaryPath = path6.join(__dirname, "../dictionaries/en_US");
  const dictionary = spellchecker.parse({
    aff: fs3.readFileSync(`${dictionaryPath}/index.aff`),
    dic: fs3.readFileSync(`${dictionaryPath}/index.dic`)
  });
  ...
  lines.forEach((line, index) => {
    ...
    if (match) {
      ...
      const words = value.split(/\s+/).flatMap((word) => word.split(/(?<=[a-z])(?=[A-Z])/)).map((m) => /^[a-zA-Z]+$/.test(m) ? m : null).filter(Boolean);
      words.forEach((word) => {
        if (!word.match(/^[A-Z]+$/) && // Ignore all uppercase words
        !ignoreWords.includes(word.toLowerCase()) && (!spellchecker.check(word) || flagWords.includes(word))) {
          results.push({
            text: `${word}`,
            line: index + 1,
      ...
      try {
        ...
        await vscode11.window.withProgress(
          ...
          async (progress, token) => {
            ...
            const config = vscode11.workspace.getConfiguration("i18nAiExtractor");
            const scanFoldersForUnused = config.get("localeResourceFolder", "src").split(",").map((folder) => folder.trim());
            const folderPaths = scanFoldersForUnused.map(
              (folder) => path7.join(vscode11.workspace.workspaceFolders[0].uri.fsPath, folder)
            );
            const scanFileExtensions = config.get("scanFileExtensions", [
              "js",
              "jsx",
            ...
            for (const folderPath of folderPaths) {
              const totalFiles = await vscode11.workspace.findFiles(
                new vscode11.RelativePattern(folderPath, filePattern)
              );
        ...
        if (results.length > 0) {
          ...
          vscode11.window.showInformationMessage("No typos found.");
        }
      } catch (error) {
...
var extractLocale = {
  bind: (context) => {
    return async (text, line, requireConfirmation = true) => {
      let editor = vscode12.window.activeTextEditor;
      if (!editor) {
        editor = vscode12.window.visibleTextEditors[vscode12.window.visibleTextEditors.length - 1];
      }
      if (!editor && (line === -1 || typeof line === "undefined")) {
        vscode12.window.showErrorMessage("No active text editor found.");
        return;
      }
      if (!text) {
      ...
      const config = vscode12.workspace.getConfiguration("i18nAiExtractor");
      ...
      const fullPath = path8.join(
        vscode12.workspace.workspaceFolders[0].uri.fsPath,
        localePath
      );
      let updatedKey;
      ...
      const i18nFunctionName = config.get("i18nFunctionName", "i18next.t");
      if (updatedKey) {
        const fileName = path8.basename(fullPath, path8.extname(fullPath));
        ...
        if (line > -1) {
          ...
          if (startIndex !== -1) {
            ...
            if (lineText[rangeStart - 1] === '"' || lineText[rangeStart - 1] === "'") {
              rangeStart--;
            }
            ...
            const range = new vscode12.Range(
              new vscode12.Position(line, rangeStart),
              new vscode12.Position(line, rangeEnd)
            );
          ...
          if (editor.selection.isEmpty) {
            ...
            if (sentenceRange) {
              ...
              const range = new vscode12.Range(
                new vscode12.Position(position.line, rangeStart),
                new vscode12.Position(position.line, rangeEnd)
              );
              editor.edit((editBuilder) => {
                editBuilder.replace(range, replacement);
              });
            ...
            editor.edit((editBuilder) => {
              ...
              if (lineText[startChar - 1] === '"' || lineText[startChar - 1] === "'") {
                startChar--;
              }
              ...
              range = new vscode12.Selection(
                new vscode12.Position(range.start.line, startChar),
                new vscode12.Position(range.end.line, endChar)
              );
              editBuilder.replace(range, replacement);
            });
          }
...
async function updateLocaleFile(filePath, key, translation, requireConfirmation = true) {
  try {
    let localeData = {};
    try {
      const data = await fs4.readFile(filePath, { encoding: "utf8" });
    ...
    if (requireConfirmation) {
      const modifiedKey = await vscode12.window.showInputBox({
...
async function suggestKeyWithOpenAI(text, apiKey, chatTemplate) {
  ...
  return await vscode12.window.withProgress(
    {
        ...
        const response = await axios_default.post(
          `${openAIBasePath}/chat/completions`,
          {
            ...
            messages: [
              {
                role: "system",
                content: "You are a helpful assistant that suggests concise i18n keys for given text."
              },
              { role: "user", content: prompt }
            ],
            max_tokens: 50
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode13.commands.registerCommand("extension.extractLocaleNoConfirm", () => {
  ...
  context.subscriptions.push(
    vscode13.commands.registerCommand("extension.openLocaleFile", async () => {
      const editor = vscode13.window.activeTextEditor;
      if (!editor) {
        vscode13.window.showErrorMessage("No active text editor found.");
        return;
      }
      const selection = editor.selection;
      let text;
      ...
      if (!i18nKeyMatch) {
        vscode13.window.showErrorMessage("No i18n expression found.");
        return;
      }
      const i18nKey = i18nKeyMatch[1];
      const config = vscode13.workspace.getConfiguration("i18nAiExtractor");
      const localePath = config.get("localePath", "");
      if (!localePath) {
        vscode13.window.showErrorMessage("Locale file path not configured.");
        return;
      }
      const fullPath = path9.join(
        vscode13.workspace.workspaceFolders[0].uri.fsPath,
        localePath
      );
      const fileName = path9.basename(fullPath, path9.extname(fullPath));
      ...
      const uri = vscode13.Uri.file(fullPath);
      const position = new vscode13.Position(lineNumber, 0);
      await vscode13.window.showTextDocument(uri, {
        selection: new vscode13.Range(position, position)
      });
    })
  );
  context.subscriptions.push(
        ...
        if (selection.isEmpty) {
          ...
          if (text.length > maxRequestSize) {
            ...
            if (choice === "Select Portion") {
              const newSelection = await vscode13.window.showTextDocument(
                document2,
                { selection: new vscode13.Selection(0, 0, 0, 0) }
              );
              if (newSelection) {
        ...
        const fileListOccurrences = [
          {
            ...
            occurrences: knownOccurrences.concat(
