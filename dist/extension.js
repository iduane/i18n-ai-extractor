...
var require_axios = __commonJS({
  "node_modules/axios/dist/node/axios.cjs"(exports2, module2) {
    ...
    function renderKey2(path4, key, dots) {
      if (!path4) return key;
    ...
    function toFormData3(obj, formData, options) {
      ...
      function defaultVisitor(value, key, path4) {
        ...
        formData.append(renderKey2(path4, key, dots), convertValue(value));
        return false;
      }
      const stack = [];
    ...
    function formDataToJSON2(formData) {
      function buildPath(path4, value, target, index) {
        ...
        const result2 = buildPath(path4, value, target[name], index);
    ...
    function assertOptions2(options, schema, allowUnknown) {
      ...
      while (i-- > 0) {
        ...
        if (validator2) {
          ...
          if (result2 !== true) {
            throw new AxiosError3("option " + opt + " must be " + result2, AxiosError3.ERR_BAD_OPTION_VALUE);
          }
          continue;
        }
...
function createWebviewPanel(title, description, fileListOccurrences, context, onCommand2) {
      ...
      (occ, occIndex) => `
                  <tr class="${(occ.options || {}).disable ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-100 cursor-pointer"}"
                    ...
                    <td class="py-2 px-4 border-b flex items-center">
                      ...
                      ${escapeHtml(occ.text)}
                    </td>                    <td class="py-2 px-4 border-b text-right">${occ.line}</td>
                  </tr>
                `
    ).join("")}
              </tbody>
...
function isFunctionParam(code, text, index) {
  ...
  const lastFunctionCall = beforeText.split("\n").reverse().find((line) => functionParamRegex.test(line));
  if (lastFunctionCall) {
...
var result = scanForUnlocalizedText(testCode, "jsx");
console.log(result);
...
async function findUnlocalizedText(text, config) {
  const outputChannel3 = createOutputChannel();
  ...
  return await vscode5.window.withProgress(
    ...
    async (progress) => {
      ...
      try {
        ...
        outputChannel3.appendLine("AI Response:");
        outputChannel3.appendLine(aiOutput);
        outputChannel3.show();
...
function parseAIOutput(aiOutput, fileContent, config) {
  ...
  unlocalizedTexts = parsedOutput.map((item) => {
    ...
    console.log("Processed item:", JSON.stringify(result2, null, 2));
    return result2;
...
async function analysisRegexpMatches(results, context, config) {
  ...
  if (openAIApiKey) {
    ...
    const allOccurrences = results.map((result2) => result2.occurrences).flat().map((occ) => occ.currentLineText);
    ...
    for (let { text } of unlocalizedTexts) {
      for (let result2 of results) {
        for (let occ of result2.occurrences) {
          if (occ.text === text) {
            occ.ai = true;
          }
        }
...
function activate(context) {
  outputChannel2 = createOutputChannel();
  context.subscriptions.push(
    vscode7.commands.registerCommand(
      "extension.extractLocale",
      async (text, line, requireConfirmation = true) => {
        let editor = vscode7.window.activeTextEditor;
        if (!editor) {
          editor = vscode7.window.visibleTextEditors[vscode7.window.visibleTextEditors.length - 1];
        }
        if (!editor && (line === -1 || typeof line === "undefined")) {
          vscode7.window.showErrorMessage("No active text editor found.");
          return;
        }
        if (!text) {
        ...
        const i18nFunctionName = config.get("i18nFunctionName", "i18next.t");
        if (updatedKey) {
          const fileName = path3.basename(fullPath, path3.extname(fullPath));
          ...
          if (line > -1) {
            ...
            if (startIndex !== -1) {
              ...
              const range = new vscode7.Range(
                new vscode7.Position(line, rangeStart),
                new vscode7.Position(line, rangeEnd)
              );
            ...
            if (editor.selection.isEmpty) {
              ...
              if (sentenceRange) {
                ...
                const range = new vscode7.Range(
                  new vscode7.Position(position.line, rangeStart),
                  new vscode7.Position(position.line, rangeEnd)
                );
                editor.edit((editBuilder) => {
                  editBuilder.replace(range, replacement);
                });
              ...
              editor.edit((editBuilder) => {
                ...
                range = new vscode7.Selection(
                  new vscode7.Position(range.start.line, startChar),
                  new vscode7.Position(range.end.line, endChar)
                );
                editBuilder.replace(range, replacement);
              });
            }
  ...
  context.subscriptions.push(
    vscode7.commands.registerCommand("extension.extractLocaleNoConfirm", () => {
  ...
  context.subscriptions.push(
    vscode7.commands.registerCommand("extension.openLocaleFile", async () => {
      ...
      if (!i18nKeyMatch) {
        vscode7.window.showErrorMessage("No i18n expression found.");
        return;
      }
      ...
      const localePath = config.get("localePath", "");
      if (!localePath) {
        vscode7.window.showErrorMessage("Locale file path not configured.");
        return;
      }
      const fullPath = path3.join(
        vscode7.workspace.workspaceFolders[0].uri.fsPath,
      ...
      await vscode7.window.showTextDocument(uri, {
        selection: new vscode7.Range(position, position)
      });
    })
  );
  context.subscriptions.push(
        ...
        if (selection.isEmpty) {
          ...
          if (text.length > maxRequestSize) {
            ...
            if (choice === "Select Portion") {
              const newSelection = await vscode7.window.showTextDocument(
                document2,
                { selection: new vscode7.Selection(0, 0, 0, 0) }
              );
              if (newSelection) {
...
async function updateLocaleFile(filePath, key, translation, requireConfirmation = true) {
  try {
    ...
    if (requireConfirmation) {
      const modifiedKey = await vscode7.window.showInputBox({
